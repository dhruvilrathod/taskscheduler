...
export class PiechartComponent implements OnInit {
  ...
  constructor(
    ...
    this.titleService.setTitle("Pie Chart")
  }
  ...
  drawpieChart() {
    this.loading = true;
    this.http.get("https://us-central1-smarttaskscheduler.cloudfunctions.net/app/api/tasks/getPieData", { headers: { 'auth-token': localStorage.getItem('auth-token') } }).subscribe((data) => {
      this.pieSection = document.getElementById('pieChartSection');
      new Chart(this.pieSection, {
        type: 'pie',
        data: {
          labels: [
            'Very High Priority',
  ...
  deleteTask() {
    ...
    this.http.delete('https://us-central1-smarttaskscheduler.cloudfunctions.net/app/api/tasks/deleteTask', { headers: { 'auth-token': localStorage.getItem('auth-token') }, params: { 'task_id': this.deleteTaskId } }).subscribe((data) => {
      ...
      this.closeModal();
      this.prioritySelectedFunction(this.event);
    }, (err) => {
      this.loading = false;
  ...
  prioritySelectedFunction(p: string) {
    this.specialDates = [];
    this.event = p;
    this.loading = true;
    ...
    //console.log(p);
    param = param.append("sort_type", p);
